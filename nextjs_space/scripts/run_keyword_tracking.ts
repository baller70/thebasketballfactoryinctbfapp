...
async function main() {
  ...
  await logMessage('========================================');
  await logMessage('Starting Weekly Keyword Ranking Tracker');
  await logMessage('========================================');
  
  try {
    // Step 1 & 2: Track keyword rankings and save changes
    await logMessage('Step 1-2: Tracking keyword rankings...');
    ...
    await logMessage(`✓ Saved ${rankingChanges.length} ranking changes to ${changesFile}`);
    
    // Step 3 & 4: Generate SEO alerts and save
    await logMessage('Step 3-4: Generating SEO alerts...');
    ...
    await logMessage(`✓ Saved ${alerts.length} alerts to ${alertsFile}`);
    
    // Step 5: Send critical alerts via email
    const criticalAlerts = alerts.filter(a => a.severity === 'critical');
    
    if (criticalAlerts.length > 0) {
      await logMessage(`Step 5: Sending ${criticalAlerts.length} critical alerts via email...`);
  ...
  <div style="padding: 20px;">
    ...
    <p><a href="https://thebasketballfactoryinc.com/admin/seo">View Full SEO Dashboard</a></p>
      ...
      if (emailResult.success) {
        await logMessage('✓ Critical alert email sent successfully');
      } else {
        await logMessage(`✗ Failed to send critical alert email: ${emailResult.error}`);
      }
    } else {
      await logMessage('Step 5: No critical alerts to send');
    }
    ...
    await logMessage('========================================');
    await logMessage('Weekly Keyword Ranking Tracker Summary');
    await logMessage('========================================');
    await logMessage(`Total keywords tracked: ${rankingChanges.length > 0 ? 'Active keywords processed' : 'No active keywords'}`);
    await logMessage(`Ranking changes detected: ${rankingChanges.length}`);
    await logMessage(`Alerts generated: ${alerts.length}`);
    await logMessage(`Critical alerts: ${criticalAlerts.length}`);
    await logMessage(`Duration: ${duration.toFixed(2)}s`);
    await logMessage(`Status: SUCCESS`);
    await logMessage('========================================\n');
    ...
    await logMessage('========================================');
    await logMessage('Weekly Keyword Ranking Tracker FAILED');
    await logMessage('========================================');
