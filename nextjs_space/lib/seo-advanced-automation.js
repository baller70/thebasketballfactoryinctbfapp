...
exports.monitorBacklinks = monitorBacklinks;
exports.generateAIContentSuggestions = generateAIContentSuggestions;
exports.setupABTesting = setupABTesting;
exports.trackCompetitors = trackCompetitors;
...
async function autoPostToSocial() {
    try {
        ...
        const recentPrograms = await db_1.prisma.sEOPageConfig.findMany({
            where: {
                status: 'active',
...
async function generateAIContentSuggestions() {
    try {
        ...
        for (const page of pages) {
            ...
            const performance = await db_1.prisma.sEOPerformance.findFirst({
 ...
 */
function generateTitleVariants(originalTitle, keywords) {
    ...
    try {
        ...
        for (const performance of lowCTRPages) {
            ...
            if (ctr < 3 && performance.pagePath) {
                const page = await db_1.prisma.sEOPageConfig.findFirst({
                ...
                if (page) {
                    ...
                    await db_1.prisma.sEOPageConfig.update({
                        where: { id: page.id },
                        data: {
