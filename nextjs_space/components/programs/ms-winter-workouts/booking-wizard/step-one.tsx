...
const packages = [
  {
    ...
    name: 'Full Program (7 Weeks)',
    price: 500,
    sessionCount: 15,
    pricePerSession: 33.33,
    features: [
      '15 training sessions (8 Sundays + 7 Mondays)',
      'Sundays: 10:00 AM - 11:30 AM (90 min)',
      'Mondays: 6:30 PM - 8:00 PM (90 min)',
  ...
  },
  {
    ...
    pricePerSession: 35,
    features: [
      '1 training session per registration',
...
export default function StepOne({ bookingData, updateBookingData, nextStep }: StepOneProps) {
  // Check if the full program registration deadline has passed (December 7, 2025)
  const today = new Date()
  const registrationDeadline = new Date('2025-12-07T23:59:59')
  const isFullProgramAvailable = today <= registrationDeadline

  const handleSelectPackage = (pkg: typeof packages[0]) => {
    // Don't allow selecting full program after the deadline
    if (pkg.id === 'full-program' && !isFullProgramAvailable) {
      return
    }
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
        {packages.map((pkg) => {
          const priceBreakdown = calculateStripeTotal(pkg.price)
          const isDisabled = pkg.id === 'full-program' && !isFullProgramAvailable
              ...
              className={`
                ...
                ${isDisabled 
                  ...
                  : bookingData.sessionOption === pkg.id 
                    ? 'bg-[#C8B273]/10 border-[#C8B273] shadow-lg shadow-[#C8B273]/20 cursor-pointer' 
                    : 'bg-black/50 border-white/20 hover:border-[#C8B273]/50 cursor-pointer'}
              `}
            >
              ...
              {bookingData.sessionOption === pkg.id && !isDisabled && (
